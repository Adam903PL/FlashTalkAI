...
const FetchFlashcardsWithIds = () => {
  const [flashcards, setFlashcards] = useState<Word[]>([]);  // Tablica flashcardów
    ...
    <div className="cardLearnContainer">
      {flashcards.length > 0 ? (
        <div className="cardLearn">
          {/* Strzałka w lewo (do poprzedniego słówka) */}
          <div className="arrow" onClick={handlePrev}>
            {"<"}
          </div>

          {/* Wyświetlanie słówka lub tłumaczenia */}
          <div className="word" onClick={handleWordClick}>
            {showTranslation ? flashcards[wordIndex].translation : flashcards[wordIndex].word}
          </div>

          {/* Strzałka w prawo (do następnego słówka) */}
      ...
      {/* Informacja o aktualnym indeksie */}
      <div className="wordIndexInfo">
        <p>
          {wordIndex + 1}/{flashcards.length} słówek
        </p>
      </div>
    </div>
  );
...
export default FetchFlashcardsWithIds;
